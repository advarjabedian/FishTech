...
def _get_display_name(django_user):
    """Get display name for a DjangoUser."""
    return django_user.get_full_name() or django_user.username
...
def operations_dashboard(request):
    ...
    # Get SOPs for the tenant, filtered by date (only SOPs that existed on selected date)
    sops = SOP.objects.filter(tenant=tenant).filter(
        Q(created_at__isnull=True) | Q(created_at__lte=selected_date)
    ).order_by('sop_did') if tenant_id else []
    ...
    existing_inspections = SOPParent.objects.filter(
        tenant=tenant,
        date=selected_date
    ).prefetch_related('children') if tenant_id else []
    ...
    for parent in existing_inspections:
        ...
        if parent.shift == 'Pre-Op':
            valid_sop_ids = list(SOP.objects.filter(tenant=tenant, pre=True).filter(
                Q(created_at__isnull=True) | Q(created_at__lte=parent.date)
            ).values_list('sop_did', flat=True))
        elif parent.shift == 'Mid-Day':
            valid_sop_ids = list(SOP.objects.filter(tenant=tenant, mid=True).filter(
                Q(created_at__isnull=True) | Q(created_at__lte=parent.date)
            ).values_list('sop_did', flat=True))
        else:
            valid_sop_ids = list(SOP.objects.filter(tenant=tenant, post=True).filter(
                Q(created_at__isnull=True) | Q(created_at__lte=parent.date)
            ).values_list('sop_did', flat=True))
    ...
    users = [{'userid': tu.user.id, 'name': _get_display_name(tu.user)} for tu in tenant_users]
    ...
    if tenant_id:
        config = CompanyOperationConfig.objects.filter(tenant=tenant).first()
    ...
    is_holiday = CompanyHoliday.objects.filter(
        tenant=tenant,
        date=selected_date
    ).exists() if tenant_id else False
...
def inspection_form(request, parent_id):
    ...
    # Get SOPs for this tenant and shift (only SOPs that existed on inspection date)
    if sop_parent.shift == 'Pre-Op':
        sops = SOP.objects.filter(tenant=request.tenant, pre=True).filter(
            Q(created_at__isnull=True) | Q(created_at__lte=sop_parent.date)
        ).order_by('zone__name', 'sop_did')
    elif sop_parent.shift == 'Mid-Day':
        sops = SOP.objects.filter(tenant=request.tenant, mid=True).filter(
            Q(created_at__isnull=True) | Q(created_at__lte=sop_parent.date)
        ).order_by('zone__name', 'sop_did')
    else:
        sops = SOP.objects.filter(tenant=request.tenant, post=True).filter(
            Q(created_at__isnull=True) | Q(created_at__lte=sop_parent.date)
        ).order_by('zone__name', 'sop_did')
...
def operations_admin(request):
    ...
    users = [{'id': tu.user.id, 'name': _get_display_name(tu.user)} for tu in tenant_users]
    ...
    holidays = set(CompanyHoliday.objects.filter(tenant=tenant).values_list('date', flat=True))
...
def print_sop_schedule(request):
    ...
    sops = SOP.objects.filter(tenant=request.tenant).select_related('zone').order_by('zone__name', 'sop_did')
...
def generate_operational_report(request, parent_id):
    ...
    elements.append(Paragraph(f"<b>Company:</b> {tenant.name}", header_style))
    ...
    for child in children:
        sop = SOP.objects.filter(sop_did=child.sop_did, tenant=request.tenant).first()
...
def generate_bulk_report(request):
    ...
    if not dates_str:
        return HttpResponse("Missing dates parameter", status=400)
    ...
    response = HttpResponse(buffer, content_type='application/pdf')
    response['Content-Disposition'] = f'attachment; filename="bulk_report_{tenant.name}_{dates[0]}_to_{dates[-1]}.pdf"'
    return response